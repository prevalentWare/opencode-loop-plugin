...
- Preferred: open a private report through
  [GitHub Security Advisories](https://github.com/prevalentWare/opencode-loop-plugin/security/advisories/new).
...
boundary are in scope, even when the underlying capability is provided by
OpenCode itself — for example, loop iterations escaping the busy/idle gating,
injected loop instructions escalating past the plan-mode deferral, or a loop
continuing to run after the user stopped it or its session was deleted.
